...
def calculate_median_angle_triangle(vertex, original_vertices):
    """Calculate the angle of the median from a vertex in a triangle with respect to the horizontal axis."""
    # Find the opposite side's midpoint
    opposite_vertices = [v for v in original_vertices if v != vertex]
    if len(opposite_vertices) != 2:
        raise ValueError("Expected two opposite vertices for a triangle.")
    opposite_vertex1, opposite_vertex2 = opposite_vertices
    midpoint = (
...
def calculate_diagonal_angle_hexagon(vertex, original_vertices):
    """Calculate the angle of a diagonal from a vertex through the center of a hexagon."""
    # Calculate the center of the hexagon
    center_x = sum(v[0] for v in original_vertices) / len(original_vertices)
    center_y = sum(v[1] for v in original_vertices) / len(original_vertices)
...
def new_shape(original_vertices, shape_type):
    """Create a new shape (triangle or hexagon) on one of the vertices."""
    new_shape_vertices = None
    for x in range(len(original_vertices)):
        point = original_vertices[x]
        if point not in CONNECTION_POINTS:
            if shape_type == 'triangle':
                angle = calculate_median_angle_triangle(point, original_vertices)
                new_shape_vertices = equilateral_triangle_from_point(point, 50, angle)
            else:
                angle = calculate_diagonal_angle_hexagon(point, original_vertices)
