...
def triangles_overlap(triangle1, triangle2):
    ...
    for vertex in triangle1:
        if vertex in CONNECTION_POINTS:
            print(f"vertex {vertex} of triangle1 {triangle1} is in CONNECTION_POINTS {CONNECTION_POINTS}")
            #continue
        else:
            if is_point_in_triangle(vertex, triangle2):
                print(f"vertex {vertex} of triangle1 {triangle1} is in triangle2 {triangle2}")
                overlaps.append(vertex)
                return True
    for vertex in triangle2:
        if vertex in CONNECTION_POINTS:
            print(f"vertex {vertex} of triangle2 {triangle2} is in CONNECTION_POINTS {CONNECTION_POINTS}")
            #continue
        else:
            if is_point_in_triangle(vertex, triangle1):
                print(f"vertex {vertex} of triangle2 {triangle2} is in triangle1 {triangle1}")
                overlaps.append(vertex)
                return True
    print(overlaps)
    return False
...
def new_triangle(triangle_vertices):
    ...
    print("Returning None as no new triangle can be created")
    return None  # Return None if no new triangle can be created
